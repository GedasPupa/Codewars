...
export function battleCodes(armyLetters: string, armyNumbers: string): string {
  let lettersArr: string[] = armyLetters.split("");
  let numbersArr: string[] = armyNumbers.split("");
  let llPower, lnPower, flPower, fnPower;
  let i = 0;
...
function getPower(symbol: string): number {
  const power = symbol.charCodeAt(0);
  if (power > 57) {
    return power - 96;
  } else {
    return power - 48;
